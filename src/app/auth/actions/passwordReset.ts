...
// Action for admins to reset a password to the user's NSU ID (manual fallback)
export async function adminResetPassword(requestId: string, userId: string) {
  try {
    ...
    try {
      await sendNoReplyEmail({
        to: user.email,
        subject: `Password Reset by Administrator - NSUone`,
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
            <h2 style="color: #4f46e5;">Password Reset by Admin</h2>
            <p>Hello ${user.name},</p>
            <p>An administrator has reset your NSUone account password.</p>
            <p>Your temporary password is your <strong>NSU ID (${user.nsuId})</strong>.</p>
            <p>Please log in immediately and update your password in your profile settings.</p>
            <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 20px 0;" />
            <p style="color: #64748b; font-size: 0.9em;">This is an automated message from NSUone. Please do not reply to this email.</p>
          </div>
        `
    ...
    return { success: true, message: 'Password has been successfully reset to their NSU ID and email notification sent.' };
  } catch (error: any) {
