...
export default function WithdrawalManager({ initialRequests }: { initialRequests: any[] }) {
  ...
  const columns: ColumnDef<WithdrawalRow>[] = [
    ...
    { header: 'Requested', accessorKey: 'amount', cell: (row) => `${row.amount.toFixed(2)} BDT` },
    { header: 'Platform Fee (5%)', accessorKey: 'platformFee', cell: (row) => `${row.platformFee.toFixed(2)} BDT` },
    {
      header: 'Net Payout',
      accessorKey: 'netAmount',
      cell: (row) => <span className="font-semibold text-primary">{row.netAmount.toFixed(2)} BDT</span>,
    },
    {
      header: 'MFS Method & Account',
      cell: (row) => (
        <div>
          <div className="text-sm font-bold" style={{ color: MFS_COLOR[row.mfsType] ?? 'var(--text-main)' }}>
            {row.mfsType}
          </div>
          <div className="font-medium text-main">{row.accountNumber}</div>
          <div className="text-xs text-muted mt-1">Type: {row.transferType}</div>
        </div>
      ),
    },
    { header: 'Request Date', accessorKey: 'createdAt' },
    {
      ...
      accessorKey: 'status',
      cell: (row) => (
        <span className={`badge ${row.status === 'PENDING' ? 'badge-primary' : (row.status === 'APPROVED' ? 'badge-success' : 'badge-danger')}`}>
          {row.status}
        </span>
      ),
    },
